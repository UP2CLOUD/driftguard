...
| DORA / NIS2 / ISO 27001 evidence in PR review | ✅ Available | `compliance/controls.py::CATALOG` maps rule IDs to real article/clause citations (DORA `Art.9`/`Art.10`/`Art.12`/`Art.16`; NIS2 `Art.21(2)(...)`; ISO27001 `A.8.x`/`A.5.x`), cited in the PR's AI review "Compliance notes". Covers ~200 Checkov rules **and**, as of `compliance/mappings.py::_NATIVE`, DriftGuard's own scanner rules — 24 of the 33 native rules across `TF00x`/`K8S00x`/`GHA00x`. The remaining 9 (`TF006`, `TF011`, `TF015`, `K8S002`, `K8S007`, `GHA003`, `GHA006`, `GHA007`, `GHA008` — hardcoded secrets, missing concurrency limits/probes/resource limits, script-injection and secrets-in-logs findings) don't map cleanly to any `CATALOG` control and are deliberately left uncited rather than mismatched. Previously every native finding cited nothing at all — see `PRODUCTION_READINESS.md` N-12 |
