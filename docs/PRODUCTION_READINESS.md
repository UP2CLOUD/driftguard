...
| N-12 | **Mapeamento de compliance (DORA/NIS2/ISO27001) só cobria findings do Checkov** | Baixa | ✅ Corrigido. `services/scanner/rules/{terraform,kubernetes,github_actions}.py` já anotava cada finding com um `controls=[...]` ad-hoc (ex.: `["access_control", "least_privilege"]`) no momento em que a regra dispara — mas essa tag nunca esteve ligada a `compliance/mappings.py`. `ai/findings.py::from_static_scan` chama `controls_for_rule(sf.rule_id)`, exactamente como faz para findings do Checkov, só que `CHECKOV_RULE_TO_CONTROLS` só continha `CKV_*`; para qualquer `TF00x`/`K8S00x`/`GHA00x` isto devolvia sempre `()`, silenciosamente, desde que o scanner nativo existe — nenhum finding nativo alguma vez citou um artigo real, independentemente da tag que a regra já carregava. `tests/test_findings.py::test_controls_populated_for_native_scanner_rule` prova a regressão exacta (TF001 sem mapeamento devolveria `controls == ()`). Corrigido com `compliance/mappings.py::_NATIVE`, mapeando 24 das 33 regras nativas para os IDs reais do `CATALOG` (não para o vocabulário ad-hoc do scanner) — cada mapeamento foi decidido lendo a descrição real da regra, não copiado às cegas: por exemplo `TF011` (Lambda sem `reserved_concurrent_executions`) e `K8S007` (sem probes) ficam **deliberadamente sem citação**, porque são achados de fiabilidade operacional, não de segurança/compliance, e forçar uma categoria do `CATALOG` que não corresponde seria pior do que não citar nada — o mesmo princípio já aplicado à cobertura parcial do Checkov. Lista completa da cobertura e das 9 regras deixadas de fora em `FEATURE_MATRIX.md`, linha "DORA / NIS2 / ISO 27001 evidence in PR review". `tests/test_compliance.py` (7 testes novos, incluindo um controlo negativo confirmando que `K8S002` — fora de escopo — continua sem controlos) |
...
Trabalho de continuação a partir dos achados deste próprio documento (secção
"Itens novos encontrados nesta revalidação"). N-12 estava marcado como
conhecido e não corrigido; passou a ✅ Resolvido — ver a linha N-12 acima
para o detalhe técnico completo. Resumo: `compliance/mappings.py` ganhou
`_NATIVE`, mapeando 24 das 33 regras do scanner próprio do DriftGuard
(`TF00x`/`K8S00x`/`GHA00x`) para os IDs reais do `CATALOG` de compliance,
corrigindo um caminho que devolvia `()` silenciosamente para todo finding
nativo desde que o scanner existe. 8 testes novos em `tests/test_compliance.py`
e `tests/test_findings.py`, incluindo um que reproduz a regressão exacta e
um controlo negativo confirmando que regras fora de escopo (fiabilidade, não
compliance) continuam deliberadamente sem citação.

Suite completa re-corrida após a mudança:

| Gate | Resultado |
|---|---|
| `ruff check` / `ruff format --check` | limpo |
| `mypy driftguard` | 0 problemas |
| `pytest` (`apps/api`, exclui `tests/eval`) | 1209 passed (+8 vs. checkpoint anterior) |
